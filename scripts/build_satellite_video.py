...
SOURCE_DIR = (
    ROOT / "telescope-extracted_frames_60fps" / "telescope-extracted_frames_60fps"
)
OUTPUT_DIR = ROOT / "assets" / "satellite"

SOURCE_FRAME_RATE = 60
OUTPUT_FRAME_RATE = 60
SLOW_FRAMERATE = 10
VIDEO_BASENAME = "satellite-background"
...
def export_frames(frame_paths: list[Path]) -> Path:
    temp_dir = OUTPUT_DIR / f"_temp_{uuid.uuid4().hex}"
    ...
    poster.save(OUTPUT_DIR / "satellite-poster.jpg", quality=92, optimize=True)
...
def main() -> None:
    ...
    print(f"Source: {len(frame_paths)} frames -> ~{expected_duration:.1f}s output")
    ...
    print(f"Built satellite assets in {OUTPUT_DIR}")
